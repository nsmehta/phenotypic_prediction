...
def decision_tree_classifier_multi(X, y_2d, X_new, to_predict):
    ...
    print("F1 score predicted for population w/o cross val", f1_score(y_test.iloc[:, 0], output_pred[:, 0], average='weighted'))
    print("F1 score predicted for sequence center w/o cross val", f1_score(y_test.iloc[:, 1], output_pred[:, 1], average='weighted'))
