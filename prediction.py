...
def cross_val(df, folds, type, classifier):
    ...
    for i in range(len(df)):
        ...
        clf = None
        if classifier == 0:
            clf = RandomForestClassifier(n_jobs=-1, random_state=0, n_estimators=10, max_features=None,
                                         criterion="gini")
        ...
        if type == 3:  # population
            ...
            pickle.dump(clf, open(model_dump_path + slash + 'population_model.sav','wb'))
            ...
            pickle.dump(clf, open(model_dump_path + slash + 'sc_model.sav','wb'))
            ...
            pickle.dump(clf, open(model_dump_path + slash + 'population_sc_model.sav','wb'))
...
def predict_population(dataframe):
    ...
    return prediction_with_tree_classifier(dataframe, 3)
...
def predict_sequence_center(dataframe):
    ...
    return prediction_with_tree_classifier(dataframe, 2)
...
def predict_population_seq_center(dataframe):
    ...
    return prediction_with_tree_classifier(dataframe, 1)
...
def create_dataframe(files, features, col_names, label_dict):
    classifier_input = list()

    for file in files:
        name = file.split('.')[0]
        data = pd.read_csv(csv_path + slash + file, usecols=col_names,
                           converters={'TPM': float, 'Length': float, 'EffectiveLength': float, 'NumReads': float})
        ...
        data_list = data_list + [classifier_population, classifier_sequence_center,
                                 classifier_population + '-' + classifier_sequence_center]
...
def main():
    ...
    if eq_class == 'False':
        ...
        df = parse_eq_classes.create_dataframe_with_eq_class(raw_path_string, csv_path, train_path, slash, label_dict, dataframe_csv_path)
